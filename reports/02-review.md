...
> **Fixed, and the struck text is stale as of 2026-08-13.** `DetailRow` is memoized (`src/components/detail/DetailChassis.jsx`, `export const DetailRow = React.memo(DetailRowBase)`), and `React.memo` now appears well beyond three sites. Batches G/H stabilised the props this finding is really about: each of the four detail screens wraps its row in a memoized component that builds its own `onPress`/`menu`, because memoizing the row while the caller still passes inline closures earns nothing — `__tests__/listRowStability.test.js` locks that. The prescribed confirmation was also carried out differently and better: `src/lib/renderCount.js` counts renders on-device, and the measurement is in the batch-H notes. Note the `native/src/` prefix above is itself rot — this repo's root *is* the native app, so paths are `src/…`.
