...
18. **`GlassBlurController.destroy()` is dead code** **[V-adj]** — `GlassViewManager` has no `onDropViewInstance` anywhere, and the only `destroy()` call site (`GlassBlurController.kt:403`) destroys BlurView's *pre-existing* `NoOpController`, not the Glass one. *Correction, and it matters:* the heartbeat does **not** keep ticking after a view drop — BlurView 2.0.6's own `onDetachedFromWindow` disarms capture unconditionally, and `postDelayed` on a detached view parks in the `HandlerActionQueue`. Real (much smaller) fallout: `blurAlgorithm.destroy()` is never called and the two staging/internal bitmaps are never released — they just wait on GC. Demoted from my initial reading. **Fixed:** `GlassViewManager.onDropViewInstance` now calls `controller.destroy()`; the comment there carries this correction so the demoted severity is not re-inflated a third time.
...
- **History.** ~~The clone is shallow (50 commits) and most of the tree lands in one squashed audit commit (`70e10b3`); … so "fixed in commit X" claims can't be verified.~~ **Corrected 2026-08-13: the history is fine and the conclusion was wrong.** `70e10b3` is not a root — it is a **graft boundary**, because `.git/shallow` exists and `git rev-parse --is-shallow-repository` returns true. It only *looks* squashed (`310 files changed`) because its parent was never fetched. `git fetch --depth=200 origin main` restores the real history: `7a7fdd7` then resolves as a commit, `git merge-base --is-ancestor 7a7fdd7 HEAD` succeeds, and `git rev-list --count 7a7fdd7..HEAD` = 163. So "fixed in commit X" claims **are** verifiable — deepen first. This error propagated: it was repeated as fact in the perf plan and twice in session notes before anyone ran `--is-shallow-repository`. The real defect it was masking is the 163-commit citation drift now recorded at the top of `docs/perf/00-architecture-map.md`. The kotlin-audio half stands: the fork still records no upstream base commit (`reports/02-review.md:157`), so fork-vs-upstream diffing remains unverifiable.
